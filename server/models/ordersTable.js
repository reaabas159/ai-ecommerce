...
export async function createOrdersTable() {
  try {
    ...
    // Indexes
    await database.query(`CREATE INDEX IF NOT EXISTS idx_orders_buyer ON orders(buyer_id);`);
    await database.query(`CREATE INDEX IF NOT EXISTS idx_orders_date ON orders(created_at);`);

    // Security
    await database.query(`ALTER TABLE orders ENABLE ROW LEVEL SECURITY;`);
